...
Auto Mode Gate applies deterministic rules before the permission judge. Version 0.4.2 retains the global restoration of the selected Pi judge model and thinking introduced in 0.4.0; version 0.1.0 does not invoke a judge.
...
Pi uses one isolated request with `tools: []`, no history, `maxRetries: 0`, cancellation, and a local deadline that does not wait for late providers. `inherit` uses the verified model-registry path; an explicit thinking level uses Pi 0.84.2's public provider transport and exact model authentication. Only one canonical JSON allow or deny response is accepted. OpenCode has no verified transport and blocks eligible cases as unavailable.

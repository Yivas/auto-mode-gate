...
Version 0.4.2 retains the global Pi permission-judge controls introduced in 0.4.0. These controls
require Pi 0.84.2. They select the secondary model used for eligible permission decisions and never change Pi's primary
conversation model or thinking level.

<CardGrid>
  <Card title="Auto" icon="approve-check-circle">
    Request or disable model-assisted decisions without changing global authorization.
  </Card>
  <Card title="Judge model" icon="setting">
    Search Pi's current model scope and choose the secondary model used by the gate.
  </Card>
  <Card title="Thinking" icon="information">
    Select `inherit`, `off`, or a level supported by the chosen judge model.
  </Card>
  <Card title="Shortcuts" icon="forward-slash">

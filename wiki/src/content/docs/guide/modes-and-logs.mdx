...
| Mode | Behavior |
|-|-|
| `enforce` | Denied actions block; an active Pi session may call the judge for eligible input. |
| `shadow` | Deterministic policy and logs run, but actions do not block and the judge is not called. |
| `off` | The adapter remains loaded, does not block, and does not call the judge. |

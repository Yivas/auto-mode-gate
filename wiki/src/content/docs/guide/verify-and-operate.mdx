...
In the current unreleased source, verify session state from Pi with:
...
The published package documents only the validated OpenCode, Pi, and Node baselines. Judge behavior currently belongs to unreleased source. Do not infer broader compatibility or real-model safety from a successful local or loopback run.

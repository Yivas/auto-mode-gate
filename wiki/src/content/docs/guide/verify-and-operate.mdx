...
In version `0.2.0`, verify session state from Pi with:
...
The published package documents only the validated OpenCode, Pi, and Node baselines. Version 0.2.0 includes the Pi judge, but no real-model inference was used for its validation. Do not infer broader compatibility or provider safety from a successful local or loopback run.

...
Auto Mode Gate applies one host-neutral policy before a built-in Bash tool call executes. Published version 0.1.0 blocks dangerous and ambiguous actions without invoking a model judge. The current unreleased source adds an opt-in Pi judge for a narrow eligible Git request after deterministic analysis.

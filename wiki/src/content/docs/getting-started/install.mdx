...
:::note[Published package versus current source]
Package `0.1.0` contains the deterministic gate and blocks all ambiguity. The opt-in Pi judge is implemented only in the current unreleased source checkout. The `main` manifest is private to prevent publication under the existing version. `npm pack --dry-run` previews contents and a filename, but creates no tarball and authorizes no distribution.
:::

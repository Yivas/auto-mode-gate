...
Package `0.3.0` uses host-owned policy paths and can [migrate shared `0.2.0` configuration](../migrate/) without overwriting it. Package `0.2.0` includes the opt-in Pi judge with shared policy paths. Package `0.1.0` remains deterministic-only and blocks all ambiguity.
:::

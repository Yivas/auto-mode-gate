...
OpenCode 1.18.18 has no plugin removal subcommand. Use `opencode debug config`, remove only the matching Auto Mode Gate plugin item from its resolved configuration, restart OpenCode, and confirm that the plugin no longer appears.

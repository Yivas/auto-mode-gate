...
Package `0.2.0` includes the opt-in Pi judge. Package `0.1.0` remains deterministic-only and blocks all ambiguity. Builds after `0.2.0` use host-owned policy paths and can [migrate shared configuration](../migrate/) without overwriting it.
:::

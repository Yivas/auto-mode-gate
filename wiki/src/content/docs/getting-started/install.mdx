...
Package `0.4.0` adds persistent Pi controls and retains the host-owned policy paths from 0.3.0. It can [migrate shared `0.2.0` configuration](../migrate/) without overwriting it. Package `0.1.0` remains deterministic-only and blocks all ambiguity.
:::
...
Version 0.4.0 requires Pi 0.84.2 for persistent judge controls and explicit secondary-model thinking.

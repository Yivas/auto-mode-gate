...
Introduced in 0.4.0 and unchanged in 0.4.1, Pi stores user choices separately from policy in `$PI_CODING_AGENT_DIR/auto-mode-gate-preferences.json`, with `~/.pi/agent/auto-mode-gate-preferences.json` as the fallback. The file persists requested Auto, an optional judge-model override, an optional thinking override, and shortcuts.
